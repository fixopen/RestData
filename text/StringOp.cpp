...
namespace Util::Text::StringOp {
    int ToInt(std::string const &content, size_t const mode) {
        char *endPos = nullptr;
    ...
    std::wstring ToUpper(std::wstring const &v) {
        ...
        for (wchar_t i : v) {
            result += static_cast<wchar_t>(toupper(i));
        }
    ...
    std::wstring ToLower(std::wstring const &v) {
        ...
        for (wchar_t i : v) {
            result += static_cast<wchar_t>(tolower(i));
        }

        return result;
    }

    int ToInt(std::wstring const &content, size_t const mode) {
    ...
    std::wstring FromInt(int const value) {
    ...
    std::wstring FromDouble(double const value, size_t const prec) {
        char format[16] = {0};
    ...
    std::string FromIntU(int const value) {
        char stringValue[32] = {0};
        //sprintf_s(stringValue, 32, "%d", value);
        sprintf(stringValue, "%d", value);
        return stringValue;
    ...
    std::string FromDoubleU(double const value, size_t const prec) {
        char format[16] = {0};
        //sprintf_s(format, 16, "%%.%df", prec);
        sprintf(format, "%%.%df", (int) prec);
        char stringValue[64] = {0};
    ...
    namespace {
        inline bool isSpace(std::wstring const &spaceChars, wchar_t const chr) {
            bool result = false;

            for (wchar_t spaceChar : spaceChars) {
                if (spaceChar == chr) {
                    result = true;
                    break;
                }
        ...
        inline bool isSpace(std::string const &spaceChars, char const chr) {
            bool result = false;
    ...
    std::wstring Trim(std::wstring const &value, std::wstring const &spaceChars) {
        //std::wstring result;
        size_t startPos = 0;
    ...
    std::wstring ToUnicode(std::string const &content) {
        wchar_t* buffer = (wchar_t*) calloc(content.length(), sizeof(wchar_t));
        mbstowcs(buffer, content.data(), content.length());
        std::wstring r = std::wstring(buffer);
        free(buffer);
        return r;
...
//        return result;
    ...
    std::string FromUnicode(std::wstring const &value) {
        char* buffer = (char*) calloc(value.length(), sizeof(wchar_t));
        wcstombs(buffer, value.data(), value.length());
